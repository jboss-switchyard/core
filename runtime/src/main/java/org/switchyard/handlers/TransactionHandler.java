...
public class TransactionHandler implements ExchangeHandler {
    ...
    private void evaluatePolicyCombination(Exchange exchange) throws HandlerException {
        ...
        if (propagatesRequired(exchange) && managedLocalProvided(exchange)) {
            throw new HandlerException("Invalid transaction policy : "
                    + TransactionPolicy.PROPAGATES_TRANSACTION + " is required in spite of "
                    + TransactionPolicy.MANAGED_TRANSACTION_LOCAL + " is provided.");
        }
        if (propagatesRequired(exchange) && noManagedProvided(exchange)) {
            throw new HandlerException("Invalid transaction policy : "
                    + TransactionPolicy.PROPAGATES_TRANSACTION + " is required in spite of "
                    + TransactionPolicy.NO_MANAGED_TRANSACTION + " is provided.");
        }
    }
    ...
    private void evaluateTransactionStatus(Exchange exchange) throws HandlerException {
        ...
        if (transaction != null &&  txStatus != Status.STATUS_ACTIVE) {
            throw new HandlerException("Invalid transaction status: code=" + txStatus + ": see javax.transaction.Status.");
        }
        if (transaction == null && managedGlobalProvided(exchange)) {
                throw new HandlerException("Invalid transaction status : " 
                        + TransactionPolicy.MANAGED_TRANSACTION_GLOBAL + " is provided but the transaction is not active");
        }
        if (transaction == null && managedLocalProvided(exchange)) {
            throw new HandlerException("Invalid transaction status : " 
                    + TransactionPolicy.MANAGED_TRANSACTION_LOCAL + " is provided but the transaction is not active");
        }
        if (transaction != null && noManagedProvided(exchange)) {
            throw new HandlerException("Invalid transaction status : " 
                    + TransactionPolicy.NO_MANAGED_TRANSACTION + " is provided but transaction exists");
        }
    ...
    private boolean isEligibleToSuspendTransaction(Exchange exchange) throws HandlerException {
        ...
        if (managedLocalProvided(exchange)) {
           return true;
       } else {
    ...
    private boolean managedLocalProvided(Exchange exchange) {
        return PolicyUtil.isProvided(exchange, TransactionPolicy.MANAGED_TRANSACTION_LOCAL);
    }
    ...
    private void providePropagates(Exchange exchange) {
        PolicyUtil.disprovide(exchange, TransactionPolicy.SUSPENDS_TRANSACTION);
        PolicyUtil.provide(exchange, TransactionPolicy.PROPAGATES_TRANSACTION);
    }
    
    private void provideSuspends(Exchange exchange) {
        PolicyUtil.disprovide(exchange, TransactionPolicy.PROPAGATES_TRANSACTION);
        PolicyUtil.provide(exchange, TransactionPolicy.SUSPENDS_TRANSACTION);
    }

    private void provideManagedGlobal(Exchange exchange) {
        PolicyUtil.disprovide(exchange, TransactionPolicy.MANAGED_TRANSACTION_LOCAL);
        PolicyUtil.disprovide(exchange, TransactionPolicy.NO_MANAGED_TRANSACTION);
    ...
    private void provideManagedLocal(Exchange exchange) {
        PolicyUtil.disprovide(exchange, TransactionPolicy.NO_MANAGED_TRANSACTION);
        PolicyUtil.disprovide(exchange, TransactionPolicy.MANAGED_TRANSACTION_GLOBAL);
