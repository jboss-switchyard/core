...
public class ValidateHandler extends BaseHandler {
     ...
     * @param tag identifier for this ValidateHandler instance. ValidateHandler
     * look for "Validator.tag" property which points the Validator appropriate
     * for this ValidateHandler instance.
     */
    public ValidateHandler(ValidatorRegistry registry, String tag) {
        _registry = registry;
        _tag = tag;
    }
    ...
    private Validator<?> get(Exchange exchange) {
        Property sequenceProperty = exchange.getContext().getProperty(
                Validator.class.getName() + "." + _tag, Scope.activeScope(exchange));
        if (sequenceProperty != null) {
            return _registry.getValidator((QName)sequenceProperty.getValue());
        } else {
            return null;
        }
    }
